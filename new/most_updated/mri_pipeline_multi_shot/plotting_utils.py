...
def plot_kspace_data(data_pc, output_path=None):
    ...
    for coil_idx in range(n_coils):
        ...
        plt.imshow(np.log(np.abs(data_pc[:, coil_idx, :]).T+1), aspect='equal', cmap='gray')
...
def plot_single_kspace(data, output_path=None):
    ...
    plt.imshow(np.log(data.squeeze().detach().cpu().numpy().T+1), cmap='gray')
    plt.title("Kspace")
    plt.colorbar()
    if output_path is not None:
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
