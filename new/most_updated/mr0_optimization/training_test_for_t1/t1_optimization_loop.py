...
def simulate_and_process_mri(obj_p, experiment_id, n_TI, Nread, Nphase, permvec, plot_results=False,
    ...
    Returns:
        real_images: Processed magnitude images (normalized)
        kspace: K-space data
    ...
    try:
        ...
        # Your MR operations
        graph = mr0.compute_graph(seq0, obj_p, 2048, 1e-3)
    ...
    # Create magnitude images (normalized)
    real_images = torch.abs(space)

    return real_images, kspace, space, signal
...
real_images, kspace_gt, space_gt, signal_gt = simulate_and_process_mri(
    obj_p, experiment_id, n_TI, Nread, Nphase, permvec,
    plot_results=PLOT_GROUND_TRUTH,
    plot_title="Ground Truth Data"
)

mask = (real_images[0] > 8).float()

print(f"Ground truth data generated. Image shape: {real_images.shape}")
...
# ===== OPTIMIZATION SETUP =====
optimizer = torch.optim.Adam(T1_net.parameters(), lr=0.00005)  # Reduced learning rate
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.99)

# Add batch dimension to real images
real_images_batch = real_images.unsqueeze(0)  # [1, n_TI, H, W]
...
for iteration in range(max_iterations):
    ...
    T1_predicted = T1_net(real_images_batch)
    ...
    sim_images, sim_kspace, sim_space, sim_signal = simulate_and_process_mri(
        obj_p_pred, experiment_id, n_TI, Nread, Nphase, permvec,
        plot_results=False  # No plotting during training
    )

    # Add batch dimension to match network output
    ...
    # mask_expanded = mask.unsqueeze(0).unsqueeze(0).expand_as(real_images_batch)
    # image_loss = torch.sum((real_images_batch - sim_images_batch) ** 2 * mask_expanded) / torch.sum(mask_expanded)
    ...
    # Update learning rate scheduler (FIXED: no parameter needed)
    scheduler.step()
    ...
    if iteration % 15 == 0:
        ...
        # Middle row: Real images time series
        real_imgs = real_images_batch.squeeze().detach().numpy()  # Shape: [n_TI, 32, 32]
        for t in range(n_TI):
        ...
        # Bottom row: Simulated images time series
        sim_imgs = sim_images_batch.squeeze().detach().numpy()  # Shape: [n_TI, 32, 32]
        for t in range(n_TI):
...
# Save model (FIXED: removed undefined variable)
torch.save({
...
}, 'best_t1_mapping_model.pth')

print("Model saved to 'best_t1_mapping_model.pth'")