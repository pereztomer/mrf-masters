...
%               With Nc == number of receive Channels / Coils.
    ...
    if Ndim > 4 or Ndim < 2:
        raise ValueError(
            "First input 'kspace' should have one of these shapes: Ny x Nx, Nc x Ny x Nx, or Nc x Ny x Nx x Nz")
...
def detectPFdim(smplPtrn, wasAddedCoilDim):
    ...
    for d in range(Ndim):
        ...
        if isPartialFourier[d]:
            ...
            isUpper = np.array_equal(oneCol.flatten(), np.concatenate(
                [np.ones(numSamples, dtype=bool), np.zeros(sz[d] - numSamples, dtype=bool)]))
            isLower = np.array_equal(oneCol.flatten(), np.concatenate(
                [np.zeros(sz[d] - numSamples, dtype=bool), np.ones(numSamples, dtype=bool)]))
