...
gradients combined with ramp-sampling. The sequence has been modified to work with a 128x128 matrix size
and uses a multi-shot approach to fit within the TE constraints.
"""
...
def main(plot: bool = False, write_seq: bool = False, seq_filename: str = 'epi_se_rs_128_128_multishot_pypulseq.seq'):
    # ======
    # SETUP
    # ======
    fov = 250e-3  # Define FOV and resolution
    Nx = 128  # Changed from 256 to 128
    Ny = 128  # Changed from 256 to 128
    slice_thickness = 3e-3  # Slice thickness
    n_slices = 1
    TE = 100e-3  # Increased TE to accommodate k-space traversal

    # Multi-shot EPI parameters
    n_shots = 4  # Number of shots (acceleration factor)

    pe_enable = 1  # Flag to quickly disable phase encoding (1/0) as needed for the delay calibration
    ro_os = 1  # Oversampling factor
    readout_time = 4.2e-4  # Readout bandwidth (reduced by factor of 2)
    ...
    # Set system limits - Using Siemens 3T XT gradient values
    system = pp.Opts(
        max_grad=60,  # mT/m (per axis value)
        grad_unit='mT/m',
        max_slew=200,  # T/m/s (per axis value)
        slew_unit='T/m/s',
    ...
    # Limit readout gradient amplitude to hardware limits
    max_grad_Tpm = system.max_grad / 1000  # Convert to T/m
    # Calculate minimum time for readout while respecting max gradient
    min_readout_time = k_width / max_grad_Tpm
    # Use this to determine a safe readout time
    readout_time = max(readout_time, min_readout_time * 1.1)  # Add 10% safety margin

    # Phase blip in shortest possible time
    # Calculate blip gradient parameters
    min_time_for_blip = 2 * np.sqrt(delta_k / (2 * system.max_slew))
    # Ensure we don't exceed max gradient
    peak_blip_grad = system.max_slew * min_time_for_blip / 2

    if peak_blip_grad > max_grad_Tpm:
        # Need trapezoidal blip with flat top
        rise_time = max_grad_Tpm / system.max_slew
        flat_time = (delta_k - max_grad_Tpm * rise_time) / max_grad_Tpm
        blip_duration = 2 * rise_time + flat_time
    else:
        # Can use triangular blip
        blip_duration = min_time_for_blip

    # Round up the duration to gradient raster time
    blip_duration = np.ceil(blip_duration / system.grad_raster_time) * system.grad_raster_time
    ...
    # blip duration. The area between the blips should be defined by k_width.
    gx = pp.make_trapezoid(
        channel='x',
        system=system,
        area=k_width,
        duration=readout_time + blip_duration,
        max_grad=max_grad_Tpm,  # Limit to hardware capability
    )
    ...
    # Phase encoding and partial Fourier - adjusted for multi-shot approach
    # Calculate number of PE lines per shot
    Ny_per_shot = math.ceil(Ny / n_shots)

    # PE steps prior to ky=0, excluding the central line
    Ny_pre = round(part_fourier_factor * Ny_per_shot / 2 - 1)
    # PE lines after the k-space center including the central line
    Ny_post = round(Ny_per_shot / 2 + 1)
    Ny_meas = Ny_pre + Ny_post

    # Pre-phasing gradients
    gx_pre = pp.make_trapezoid(channel='x', system=system, area=-gx.area / 2)

    # Modified for multi-shot EPI
    # For each shot, we need a different initial phase encoding position
    def make_shot_prephaser(shot_idx):
        # Calculate the starting ky position for this shot
        ky_start = Ny_pre * delta_k + shot_idx * Ny_per_shot * delta_k
        gy_pre = pp.make_trapezoid(channel='y', system=system, area=ky_start)
        return gy_pre

    # Create the first shot's prephaser (we'll create others during sequence construction)
    gy_pre = make_shot_prephaser(0)
    ...
    for s in range(n_slices):
        ...
        # For multi-shot EPI, we need to run multiple excitation/acquisition blocks
        for shot in range(n_shots):
            # Recalculate the y prephaser for each shot
            gy_pre = make_shot_prephaser(shot)
            gy_pre = pp.make_trapezoid('y', system=system, area=gy_pre.area, duration=pp.calc_duration(gx_pre, gy_pre))
            gy_pre.amplitude = gy_pre.amplitude * pe_enable
            gy_pre.delay = pp.calc_duration(rf180)
    ...
    if write_seq:
        ...
        seq.set_definition(key='Name', value='epi_multishot')
