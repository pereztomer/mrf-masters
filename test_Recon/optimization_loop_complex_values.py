...
from simulate_and_process_complex_values import simulate_and_process_mri
...
# seq_path_with_reference = r"C:\Users\perez\Desktop\mrf-motion\sequence_generation\gre_epi_192_fourier_factor_1_w_reference.seq"
# seq_path_without_reference = r"C:\Users\perez\Desktop\mrf-motion\sequence_generation\gre_epi_192_fourier_factor_1_wo_reference.seq"
...
sensitivity_maps = phantom.coil_sens.squeeze().to(device)
...
magnitude = torch.abs(time_series_shots)  # Shape: [50, H, W]
phase = torch.angle(time_series_shots)  # Shape: [50, H, W]

stacked = torch.stack([magnitude, phase], dim=1)  # Shape: [50, 2, H, W]
time_series_shots = stacked.flatten(0, 1)

# ===== DEFINE NETWORK =====
from models.conv_unet import create_3d_unet_mri
...
# ===== NORMALIZE IMAGES ========
reshaped = time_series_shots.view(100, -1)  # [100, 1296]

# Calculate mean and std along the spatial dimensions (dim=1)
mean = reshaped.mean(dim=1, keepdim=True)  # [100, 1]
std = reshaped.std(dim=1, keepdim=True)  # [100, 1]

# Normalize
normalized_reshaped = (reshaped - mean) / (std + 1e-8)  # Add small epsilon to avoid div by zero

# Reshape back to original dimensions
time_series_shots_normalized = normalized_reshaped.view(100, Nx, Nx)

# ===== ADD BATCH DIMENSION =====
time_series_shots_normalized = time_series_shots_normalized.unsqueeze(0)  # shape: [1, 100, H, W]
...
for iteration in range(epochs):
    ...
    sim_images = simulate_and_process_mri(obj_p=obj_p_pred,
                                          seq_file_path=seq_path_without_reference,
    ...
    sim_images_magnitude = torch.abs(sim_images)  # Shape: [50, H, W]
    sim_images_phase = torch.angle(sim_images)  # Shape: [50, H, W]

    sim_images = torch.stack([sim_images_magnitude, sim_images_phase], dim=1)  # Shape: [50, 2, H, W]
    sim_images = sim_images.flatten(0, 1)


    # Compute loss
    image_loss = F.mse_loss(time_series_shots, sim_images)
    ...
    plot_training_results(iteration, epochs, losses,
                          ...
                          time_series_shots, sim_images_magnitude.unsqueeze(0),
                          plots_output_path,
