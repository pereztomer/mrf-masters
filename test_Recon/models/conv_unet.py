...
class UNet3D(nn.Module):
    def __init__(self, in_ch=1, out_ch=3, features=[64, 128, 256, 512], bilinear=True, registration_map=False):
        super().__init__()
        self.registration_map = registration_map
        ...
        # Add tanh activation for registration maps
        if self.registration_map:
    ...
    def forward(self, x):
        ...
        # Apply tanh for registration maps
        if self.registration_map:
            x = self.activation(x)
...
def create_3d_unet_mri(time_steps, input_shape, out_channels, model_size="medium", registration_map=False):
    """
    Create 3D U-Net for MRI parameter mapping or registration

    Args:
        time_steps: Number of time steps (time dimension)
        input_shape: Tuple of (height, width) for spatial dimensions
        out_channels: Number of output channels (e.g., 3 for T1, T2, PD or 98 for registration)
        model_size: Model size - "tiny", "small", "medium", "large", "huge"
        registration_map: If True, applies tanh activation for bounded displacement fields
    """
    configs = {
    ...
    return UNet3D(in_ch=1, out_ch=out_channels, features=config["features"],
                  bilinear=config["bilinear"], registration_map=registration_map)


# Usage example
if __name__ == "__main__":
    # Create model for parameter mapping (without tanh)
    model_param = create_3d_unet_mri(
        time_steps=50,  # Your time dimension
        input_shape=(72, 72),  # Your spatial dimensions
        out_channels=3,  # T1, T2, PD outputs
        model_size="medium",
        registration_map=False  # No tanh activation
    )

    # Create model for registration (with tanh)
    model_reg = create_3d_unet_mri(
        time_steps=50,  # Your time dimension
        input_shape=(72, 72),  # Your spatial dimensions
        out_channels=98,  # 49*2 for displacement fields
        model_size="medium",
        registration_map=True  # Tanh activation applied
    )

    # Test with your exact input shape
    x = torch.randn(1, 50, 36, 36)  # [batch, time, height, width]

    # Test parameter mapping model
    y_param = model_param(x)
    print(f"Parameter mapping - Input: {x.shape} -> Output: {y_param.shape}")  # Should be [1, 3, 36, 36]

    # Test registration model
    y_reg = model_reg(x)
    print(f"Registration - Input: {x.shape} -> Output: {y_reg.shape}")  # Should be [1, 98, 36, 36]
    print(f"Registration output range: [{y_reg.min().item():.3f}, {y_reg.max().item():.3f}]")  # Should be [-1, 1]

    # Count parameters
    params_param = sum(p.numel() for p in model_param.parameters())
    params_reg = sum(p.numel() for p in model_reg.parameters())
    print(f"Parameter mapping model - Parameters: {params_param:,} ({params_param / 1e6:.1f}M)")
    print(f"Registration model - Parameters: {params_reg:,} ({params_reg / 1e6:.1f}M)")